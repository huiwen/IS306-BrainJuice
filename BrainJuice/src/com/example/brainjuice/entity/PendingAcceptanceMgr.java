...
public class PendingAcceptanceMgr {
	...
	public void add(String userAsked, String qn, String userReplied, String answer, ArrayList<String> comment){
		pendingAcceptanceList.add(new PendingAcceptance(userAsked, qn, userReplied, answer, comment));
	}
	
	public void add(PendingAnswer pa, String userReplied, String answer){
		add(pa.getUserAsked(), pa.getQn(), userReplied, answer, pa.getComment());
	}
	
	public void deleteAccepted(String userAsked, String qn, String userReplied, String answer){
		for(PendingAcceptance temp: pendingAcceptanceList){
	...
	public PendingAnswer deleteRejected(String userAsked, String qn, String userReplied, String answer){
		for(PendingAcceptance temp: pendingAcceptanceList){
			if(temp.getQn().equals(qn) && temp.getAnswer().equals(answer) && temp.getUserAsked().equals(userAsked) && temp.getUserReplied().equals(userReplied)){
				pendingAcceptanceList.remove(temp);
				temp.getComment().add(temp.getUserReplied());
				return new PendingAnswer(temp.getUserAsked(), temp.getQn(), temp.getComment());
			}
		}
		return null;
	}
